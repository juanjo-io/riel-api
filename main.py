...
def dashboard_portfolio(lender: str = Depends(get_current_lender)):
    """Portfolio data for the dashboard — cookie-only, no API key required."""
    from providers.mock_provider import (
        MockProvider, MOCK_MERCHANTS_AR, MOCK_REVIEW_STATE_AR,
        MOCK_OVERRIDES_AR, MOCK_CASE_LOG_AR,
    )
    dp = MockProvider()   # Argentina dashboard always uses mock data
    merchants = []
    for link_id, meta in MOCK_MERCHANTS_AR.items():
        try:
            txs = dp.get_transactions(link_id)
...
@app.get("/dashboard/merchant/{link_id}")
def dashboard_merchant(link_id: str, lender: str = Depends(get_current_lender)):
    """Merchant detail data — cookie-only, no API key required."""
    from providers.mock_provider import (
        MockProvider, MOCK_MERCHANTS_AR, MOCK_REVIEW_STATE_AR,
        MOCK_OVERRIDES_AR, MOCK_CASE_LOG_AR,
    )
